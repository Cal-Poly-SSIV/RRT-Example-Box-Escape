...
for i=2:length(P)
    plot([P(1,i);P(1,i-1)],[P(2,i);P(2,i-1)],'-xr','LineWidth',3);
end
...
for i=2:length(P)
    plot([P(1,i);P(1,i-1)],[P(2,i);P(2,i-1)],'-xg','LineWidth',3);
end
...
%p is the node coords, and i is the index of previous node which is the
%closest node
node.p = p;
node.iPrev = iPrev;
rrt{end+1} = node;
...
%     p(2,1) = floor(p(2,1)*16-4);
    p(1,1) = p(1,1)*10-5;
    p(2,1) = p(2,1)*15-4; %notice he cheats here, he only searches inside the side boundaries!
    %instead of checking for collisions with the side walls.
    p
    rob.p = p;
    ...
    if (dist < param.thresh)
        ...
        beep
            prompt='go';
    pausejunk=input(prompt); %%%%%%%%%%%%%%%%%%%%%%%%%%%
        
   
        % construct P, starting at end and following parent nodes to
        % beginning
        i = length(rrt);
        P(:,1) = rrt{i}.p;
        while 1
...
% check if node is collision with obstacle
global checkcount;
...
% check if note is collision with edge
col = 0;
d = norm(p1 - p2);
m = ceil(d/res);
t = linspace(0,1,m);
